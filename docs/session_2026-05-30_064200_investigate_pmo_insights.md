...
- Selected option to dynamically update the email subject line to start with the date, followed by "Grant Intelligence" and then the suggested dynamic title, for example: `[Date] — 🇨🇦 Grant Intelligence — [Suggested Title]`.
...
- Implemented dynamic, date-first subject line customization in [mail_sender.py](file:///c:/dev/canadian-grant-intelligence/scripts/src/api/mail_sender.py).
